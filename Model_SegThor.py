...
class DownTransition(nn.Module):
    def __init__(self, inChans, nConvs, elu, dropout=False):
        ...
        outChans = 2*inChans # the channel needs to be doubled in each stage
        ...
        self.ops = _make_nConv(outChans, nConvs, elu) # multiple (conv + bn + relu)

    def forward(self, x):
        down = self.relu1(self.bn1(self.down_conv(x)))
        out = self.do1(down)
        out = self.ops(out)
        out = self.relu2(torch.add(out, down)) # Res?
        return out
...
class UpTransition(nn.Module):
    ...
    def forward(self, x, skipx):
        # x is the input of the stage
        # skipx is the yellow arrow input
        out = self.do1(x)
